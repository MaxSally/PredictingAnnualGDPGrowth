...
SingleLongTermmodel.compile(optimizer="adam", loss='mae', metrics = [tf.keras.metrics.RootMeanSquaredError()])
clone_weights = SingleLongTermmodel.get_weights()
...
# Designated code: Capital Invested = 0
#                  Labor Force = 1
#                  Fixed Broadband = 2
#                  R&D = 3
#                  Property rights = 4
...
designated_Code = ["Capital Investment", "Labor Force", "Fixed Broadband", "R&D", "Property rights", "Freedom from Corruption",
                  "Fiscal Freedom", "Business Freedom", "Labor Freedom", "Monetary Freedom", "Trade Freedom", "Investment Freedom",
                  "Financial Freedom", "Overall Economic Freedom", "Pop Above 65", "Savings as GDP"]

country = ["Canada", "Greece", "JPN", "Malaysia", "SouthAfrica", "Spain",
            "UK", "USA"]
...
# Use this one and change where necessary
# Replace the lists names with relevant countries. So if you are looking at USA, and not JPN,
# Delete JPN and replace with USA_testRMSE
Canada_test_MAE_RMSE = []
Greece_test_MAE_RMSE = []
JPN_test_MAE_RMSE = []
Malaysia_test_MAE_RMSE = []
SouthAfrica_test_MAE_RMSE = []
Spain_test_MAE_RMSE = []
UK_test_MAE_RMSE = []
USA_test_MAE_RMSE = []
...
# Will print out the test results for MAE and RMSE

# Slicing for Canada
for i in range(16):
    training_input_short_term_slice = training_input_short_term[0:11] # refers to slicing for country training data
    training_input_long_term1 = trainLV[i][0:11]   # The i is the designated code
    ...
    testing_input_long_term1 = testLV[i][0:3]
    testing_output_slice = training_output[0:3]

    history = SingleLongTermmodel.fit([training_input_short_term_slice, training_input_short_term_slice,     training_input_long_term1], training_output_slice, epochs = 100, batch_size = 1, verbose=0)
    history2 = SingleLongTermmodel.evaluate(x=[testing_input_short_term_slice, testing_input_short_term_slice, testing_input_long_term1], y=testing_output_slice, batch_size=1, verbose=2)
    Canada_test_MAE_RMSE.append([designated_Code[i], round(history2[0], 4), round(history2[1], 4)]) # Replace with another relevant country
    SingleLongTermmodel.set_weights(clone_weights) # Resets the weights back to beginning
    

# Slicing for Greece
for i in range(16):
    training_input_short_term_slice = training_input_short_term[11:22] # refers to slicing for country training data
    training_input_long_term1 = trainLV[i][11:22]   # The i is the designated code
    training_output_slice = training_output[11:22]
    
    testing_input_short_term_slice = testing_input_short_term[3:6] # 0:3 refers to slicing for country test data
    testing_input_long_term1 = testLV[i][3:6]
    testing_output_slice = training_output[3:6]

    history = SingleLongTermmodel.fit([training_input_short_term_slice, training_input_short_term_slice,     training_input_long_term1], training_output_slice, epochs = 100, batch_size = 1, verbose=0)
    history2 = SingleLongTermmodel.evaluate(x=[testing_input_short_term_slice, testing_input_short_term_slice, testing_input_long_term1], y=testing_output_slice, batch_size=1, verbose=2)
    Greece_test_MAE_RMSE.append([designated_Code[i], round(history2[0], 4), round(history2[1], 4)]) # Replace with another relevant country
    SingleLongTermmodel.set_weights(clone_weights) # Resets the weights back to beginning
...
# Slicing for Japan
for i in range(16):
    training_input_short_term_slice = training_input_short_term[22:33] # refers to slicing for country training data
    training_input_long_term1 = trainLV[i][22:33]   # The i is the designated code
    training_output_slice = training_output[22:33]
    
    testing_input_short_term_slice = testing_input_short_term[6:9] # 0:3 refers to slicing for country test data
    testing_input_long_term1 = testLV[i][6:9]
    testing_output_slice = training_output[6:9]

    history = SingleLongTermmodel.fit([training_input_short_term_slice, training_input_short_term_slice,     training_input_long_term1], training_output_slice, epochs = 100, batch_size = 1, verbose=0)
    history2 = SingleLongTermmodel.evaluate(x=[testing_input_short_term_slice, testing_input_short_term_slice, testing_input_long_term1], y=testing_output_slice, batch_size=1, verbose=2)
    JPN_test_MAE_RMSE.append([designated_Code[i], round(history2[0], 4), round(history2[1], 4)]) # Replace with another relevant country
    SingleLongTermmodel.set_weights(clone_weights) # Resets the weights back to beginning
    
# Slicing for Malaysia
for i in range(16):
    training_input_short_term_slice = training_input_short_term[33:44] # 0:11 refers to slicing for country training data
    training_input_long_term1 = trainLV[0][33:44]   # The 3 is the designated code
    training_output_slice = training_output[33:44]
    
    testing_input_short_term_slice = testing_input_short_term[9:12] # 0:3 refers to slicing for country test data
    testing_input_long_term1 = testLV[0][9:12]
    testing_output_slice = training_output[9:12]

    history = SingleLongTermmodel.fit([training_input_short_term_slice, training_input_short_term_slice, training_input_long_term1], training_output_slice, epochs = 100, batch_size = 24, verbose=0)
    history2 = SingleLongTermmodel.evaluate(x=[testing_input_short_term_slice, testing_input_short_term_slice, testing_input_long_term1], y=testing_output_slice, batch_size=2, verbose=2)
    Malaysia_test_MAE_RMSE.append([designated_Code[i], round(history2[0], 4), round(history2[1], 4)]) # Replace with another relevant country
    SingleLongTermmodel.set_weights(clone_weights) # Resets the weights back to beginning
...
Canada_test_MAE_RMSE = np.array(Canada_test_MAE_RMSE)
Greece_test_MAE_RMSE = np.array(Greece_test_MAE_RMSE)
JPN_test_MAE_RMSE = np.array(JPN_test_MAE_RMSE)
Malaysia_test_MAE_RMSE = np.array(Malaysia_test_MAE_RMSE)


print("For Canada: ") 
print("Long term Variable  MAE RMSE")
print(Canada_test_MAE_RMSE)
print("\n")

print("For Greece: ")  
print("Long term Variable  MAE RMSE")
print(Greece_test_MAE_RMSE)
print("\n")

print("For Japan: ") 
print("Long term Variable  MAE RMSE")
print(JPN_test_MAE_RMSE)
print("\n")

print("For Malaysia: ") 
print("Country Long term Variable  MAE RMSE")
print(Malaysia_test_MAE_RMSE)
#print(Malaysia_testRMSE)
